...
//             className="flex flex-col items-center gap-1 min-w-[72px] text-xs text-gray-700 hover:text-fuchsia-600 cursor-pointer"
//           >
//             <div className="w-12 h-12 rounded-full bg-fuchsia-50 flex items-center justify-center text-lg shadow-sm">
//               {cat.icon}
//             </div>
...
const CATEGORIES = [
  ...
  { key: "home_furniture", icon: "🛋️" },
  { key: "appliances", icon: "🧺" },
  { key: "travel", icon: "✈️" },
  { key: "beauty_more", icon: "💄" }
];
    ...
    <div className="border-b border-gray-200 bg-white">
      <div className="container-max flex gap-6 overflow-x-auto py-3">
        {CATEGORIES.map((cat) => (
          <Link
            key={cat.key}
            to={`/category/${encodeURIComponent(t(`categories.${cat.key}`))}`}
            className="flex flex-col items-center gap-1 min-w-[72px] text-xs text-gray-700 hover:text-fuchsia-600 cursor-pointer"
          >
            <div className="w-12 h-12 rounded-full bg-fuchsia-50 flex items-center justify-center text-lg shadow-sm">
              {cat.icon}
