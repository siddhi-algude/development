...
// /**
//  * Fetch all products from the public/products.json file
//  * Works in local dev + production builds
//  */
...
//     if (!Array.isArray(data)) {
//       throw new Error("products.json must return an array of products");
//     }
...
//   const all = await getProducts();       // load all products
//   return all.find((p) => p.id === Number(id));   // return only the requested product
// };
...
export const getProducts = async () => {
  ...
  if (!res.ok) throw new Error("Failed loading products.json");

  const data = await res.json();

  if (!Array.isArray(data)) throw new Error("products.json must return an array");
